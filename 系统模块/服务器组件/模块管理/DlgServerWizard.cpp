...
BOOL CDlgServerOptionItem1::OnInitDialog()
{
	...
	{
		SetDlgItemInt(IDC_SERVER_LEVEL, 2);
	}
	...
	m_ToolTipCtrl.AddTool(GetDlgItem(IDC_SERVER_LEVEL), _T("游戏等级 ，最大9999"));
	...
	m_ToolTipCtrl.AddTool(GetDlgItem(IDC_SERVICE_REVENUE), _T("设置游戏税收或者服务费，服务器部分游戏无效，推荐使用税收，税收为千分比，服务费为实际值"));
...
VOID CDlgServerOptionItem1::InitCtrlWindow()
{
	...
	WORD wServerType[2] = { GAME_GENRE_GOLD, GAME_GENRE_PERSONAL };// , GAME_GENRE_SCORE, GAME_GENRE_MATCH, GAME_GENRE_EDUCATE};
	LPCTSTR pszServerType[2] = { TEXT("财富类型"), TEXT("约战类型") };//, TEXT("点值类型"), TEXT("比赛类型"), TEXT("练习类型") };
	...
	pComboBox->SetCurSel(nServerTypeSel);
	return;
}
...
BOOL CDlgServerOptionItem2::OnInitDialog()
{
	...
	m_ToolTipCtrl.AddTool(GetDlgItem(IDC_ALLOW_AVERT_CHEAT_MODE), _T("防作弊模式开关,防作弊模式下，用户不能自主选桌，等待系统分配"));
...
BOOL CDlgServerOptionItem5::OnInitDialog()
{
	...
	for (int i = 0; i < FEE_OPTION_COUNT; i++)
	{
		...
		//		pGameMode->SetItemData(pGameMode->InsertString(j, pszGameMode[j]), j);
		//	}			
			pGameMode->SetItemData(pGameMode->InsertString(0, pszGameMode[2]), 2);
		}
		else
		{
	...
	m_ToolTipCtrl.AddTool(GetDlgItem(IDC_PERSONAL_ROOM_TAX), _T("私人房独立的税收，和通用税收共同计算"));
...
bool CDlgServerOptionItem5::SaveInputInfo()
{
	...
	for (int i = 0; i < FEE_OPTION_COUNT; i++)
	{
		...
		CComboBox * pGameMode = (CComboBox *)GetDlgItem(IDC_COMBO_SELECT_MODE1 + i);
		m_pCreateRoomRightAndFee->stCreateRoomPayFee[i].cbGameMode = pGameMode->GetCurSel();
	}
...
bool CDlgServerOptionItem5::GetPersonalRoomInfo(DWORD dwKindID)
{
	...
	m_pPersonalRoomOption->wBeginFreeTime  = IniData.ReadInt(TEXT("Option"), TEXT("BeginFreeTime"), 0);
	m_pPersonalRoomOption->wEndFreeTime  = IniData.ReadInt(TEXT("Option"), TEXT("EndFreeTime"), 0);
...
bool CDlgServerWizardItem1::SaveInputInfo()
{
	...
	if (m_GameServiceManager.CreateInstance()==false)
	{
		//构造提示
		TCHAR szString[512]=TEXT("");
		_sntprintf_s(szString, CountArray(szString), TEXT("[ %s ] 服务组件加载失败，创建游戏房间失败【%s】"), pGameModuleInfo->szGameName, m_GameServiceManager.GetErrorDescribe());
