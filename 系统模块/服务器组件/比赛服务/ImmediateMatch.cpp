			...
			if (pMatchSignupResult->cbSignupMode==SIGNUP_MODE_SIGNUP_FEE)
			{
				...
				if(m_pMatchOption->cbDeductArea==DEDUCT_AREA_SERVER)
				{
					...
					if (m_pMatchOption->lSignupFee>0)
					{
						...
						_sntprintf_s(szDescribe,CountArray(szDescribe),TEXT("退赛成功，退还报名费%I64d%s！"),m_pMatchOption->lSignupFee,pszFeeType[m_pMatchOption->cbFeeType-FEE_TYPE_GOLD]);
					}
					else
					{
						_sntprintf_s(szDescribe,CountArray(szDescribe),TEXT("退赛成功！"));
					}
...
bool CImmediateMatch::OnEventUserJoinMatch(IServerUserItem * pIServerUserItem, BYTE cbReason,DWORD dwSocketID)
{
	...
	if (wTableCount<m_pImmediateMatch->wStartUserCount/CImmediateGroup::m_wChairCount)
	{
		//构造消息
		TCHAR szString[128];
		_sntprintf_s(szString,CountArray(szString),TEXT("抱歉，当前桌子不够请等待。还差 %d 张桌子可以报名"),m_pImmediateMatch->wStartUserCount/CImmediateGroup::m_wChairCount-wTableCount);
