...
//房卡支付
bool CPersonalTableFrameHook::PersonalPayRoomCard(DWORD dwUserID, WORD wTableID, WORD wChairCount, DWORD dwPersonalRoomID)
{
	if(m_pPersonalRoomEventSink!=NULL)
	{
		return m_pPersonalRoomEventSink->PersonalPayRoomCard(dwUserID, wTableID, wChairCount, dwPersonalRoomID);
	}
	return false;
}
