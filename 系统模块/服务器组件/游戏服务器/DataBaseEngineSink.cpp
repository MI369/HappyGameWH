...
bool CDataBaseEngineSink::OnRequestWritePersonalGameScore(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
	...
	try
	{
		...
		CTraceService::TraceString(TEXT("约战房间写分 1"), TraceLevel_Debug);
		TCHAR   szVideoID[50];

		/*int iLength = MultiByteToWideChar(CP_ACP, 0, pWriteGameScore->szVideoNumber, strlen(pWriteGameScore->szVideoNumber) + 1, NULL, 0);
		MultiByteToWideChar(CP_ACP, 0, pWriteGameScore->szVideoNumber, strlen(pWriteGameScore->szVideoNumber) + 1, szVideoID, iLength);*/
		lstrcpyn(szVideoID, pWriteGameScore->szVideoNumber, CountArray(szVideoID));
		CTraceService::TraceString(szVideoID, TraceLevel_Debug);
		if (CServerRule::IsSuportPlayBack(m_pGameServiceOption->dwServerRule))
		{
			CTraceService::TraceString(TEXT("约战房间写分 2"), TraceLevel_Debug);
			if (pWriteGameScore->szVideoNumber[0] != 0)
			{
				CTraceService::TraceString(TEXT("约战房间写分 3"), TraceLevel_Debug);
				//写房间回放表
				//构造参数
...
bool CDataBaseEngineSink::OnRequestWriteVideoPlayerInfo(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
	...
	{
		TCHAR szInfo[260] = {0};
		wsprintf(szInfo, TEXT("OnRequestWriteVideoPlayerInfo 函数中有错误") );
		CTraceService::TraceString(szInfo,TraceLevel_Exception);
	}
	return true;
}
...
bool CDataBaseEngineSink::OnRequestWriteVideoDataInfo(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
	...
	{
		TCHAR szInfo[260] = {0};
		wsprintf(szInfo, TEXT("OnRequestWriteVideoDataInfo 函数中有错误") );
		CTraceService::TraceString(szInfo,TraceLevel_Exception);
	}
