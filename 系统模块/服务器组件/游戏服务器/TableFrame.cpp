...
bool CTableFrame::ConcludeGame(BYTE cbGameStatus, BOOL bPersonalLoop)
{
	...
	if (m_bGameStarted==false)
	{
		for (WORD i=0;i<m_wChairCount;i++)
		{
			...
			if (((m_pGameServiceOption->lMinTableScore!=0L) && pIServerUserItem->GetUserScore()<m_pGameServiceOption->lMinTableScore * m_lCellScore/m_pGameServiceOption->lCellScore) && ((m_pGameServiceOption->wServerType & GAME_GENRE_PERSONAL) != 0))
			{
				...
				{
					_sntprintf_s(personalRoomMessage.szMessage,CountArray(personalRoomMessage.szMessage),TEXT("您的游戏币少于 ") SCORE_STRING TEXT("，不能继续游戏！"),m_pGameServiceOption->lMinTableScore* m_lCellScore/m_pGameServiceOption->lCellScore);
	...
	{
		//第一局结束扣除创建消耗的房卡
		if(m_dwPersonalPlayCount == 1 && !m_bIsCancelBeforeOneTurn)
		{
			if (m_pGameServiceOption->wServerType == GAME_GENRE_PERSONAL &&  m_dwPersonalRoomID != 0)
				{
					if (m_pIPersonaTableFrameHook)
					{
						...
						{
							m_pIPersonaTableFrameHook->PersonalPayRoomCard(m_dwTableOwnerID, m_wTableID, m_wChairCount, m_dwPersonalRoomID);
						}
						else
						{
...
{
	//变量定义
	CMD_GR_RoomCheat RoomCheat;
	ZeroMemory(&RoomCheat, sizeof(RoomCheat));
	RoomCheat.wSubCmdID = wSubCmdID;
	RoomCheat.wTableID = m_wTableID;

	m_pIMainServiceFrame->SendDataBatchToCheatUser(pIServerUserItem, MDM_GR_USER, SUB_GR_USER_ROOM_CHEAT, &RoomCheat, sizeof(CMD_GR_RoomCheat));
	return true;
}
//发送数据
bool CTableFrame::SendRoomData(IServerUserItem * pIServerUserItem, WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
	...
	pRoomCheat->wSubCmdID = wSubCmdID;
	pRoomCheat->wTableID = m_wTableID;

	//作弊信息
	if (wDataSize != 0) CopyMemory(cbBuffer + sizeof(CMD_GR_RoomCheat), pData, wDataSize);
	//发送数据
	WORD wSendSize = sizeof(CMD_GR_RoomCheat) + wDataSize;

	m_pIMainServiceFrame->SendDataBatchToCheatUser(pIServerUserItem, MDM_GR_USER, SUB_GR_USER_ROOM_CHEAT, cbBuffer, wSendSize);
	return true;
}
...
bool CTableFrame::OnEventUserOffLine(IServerUserItem * pIServerUserItem)
{
	...
	if (cbUserStatus!=US_LOOKON)
	{
		...
		if ( ((cbUserStatus == US_PLAYING) && (m_wOffLineCount[wChairID]<MAX_OFF_LINE) && (m_pGameServiceOption->wServerType != GAME_GENRE_PERSONAL))
			...
			(cbUserStatus >= US_SIT && cbUserStatus <= US_PLAYING || m_PersonalTableParameter.cbIsJoinGame && m_dwTableOwnerID == pUserInfo->dwUserID || 
...
bool CTableFrame::PerformStandUpAction(IServerUserItem * pIServerUserItem, bool bInitiative/*=false*/)
{
	...
	if (
		...
		|| (m_pGameServiceOption->wServerType == GAME_GENRE_PERSONAL && cbUserStatus >= US_PLAYING)
		||
		(
		...
		(m_PersonalTableParameter.cbIsJoinGame || m_dwPersonalPlayCount != 0 || (cbUserStatus >= US_SIT && cbUserStatus <= US_PLAYING)) && !m_bIsDissumGame &&//如果房主参与游戏,且不是强制解散游戏
	...
	{	
		...
		if((m_pGameServiceOption->wServerType==GAME_GENRE_MATCH && m_pGameMatchOption->cbMatchType==MATCH_TYPE_IMMEDIATE)||
			...
			(m_dwPersonalPlayCount != 0  && (
			( 
			(m_cbGameMode==0 && m_dwDrawCountLimit != 0 && m_dwDrawCountLimit >= m_dwPersonalPlayCount) ||
			(m_cbGameMode == 2 && m_dwDrawCountLimit != 0 && m_dwDrawCountLimit >= m_cbLoopCount) ||
			(m_dwDrawTimeLimit != 0 && m_dwDrawTimeLimit >= dwTimeNow - m_dwTimeBegin)
			) 
			)
			)))
	...
	if (pIServerUserItem==pITableUserItem)
	{
		...
		{	
			...
			{
				KillGameTimer(IDI_PERSONAL_BEFORE_BEGAIN_TIME);
				m_pIMainServiceFrame->CancelCreateTable(m_dwTableOwnerID, m_dwDrawCountLimit, m_dwDrawTimeLimit, CANCELTABLE_REASON_PLAYER, m_wTableID, m_dwPersonalRoomID);
			}
		...
		{
			m_pIMainServiceFrame->UpdateTablePlayerCount(m_pGameServiceOption->wServerID, m_wTableID, m_wUserCount);
		}
...
bool CTableFrame::PerformLookonAction(WORD wChairID, IServerUserItem * pIServerUserItem)
{
	if (m_pGameServiceOption->wServerType == GAME_GENRE_PERSONAL)
	{
		SendRequestFailure(pIServerUserItem, TEXT("约战房，不能旁观！"), REQUEST_FAILURE_NORMAL); 
		return false;
	}
	//效验参数
...
bool CTableFrame::CancelTableRequest(DWORD dwUserID, WORD dwChairID)
{
	...
	SetGameTimer(IDI_TIME_CANCEL, TIME_CANCEL,1,NULL);
...
bool CTableFrame::CancelTableRequestReply(DWORD dwUserID, BYTE cbAgree)
{
	...
	if(nCancelCount > ((m_wChairCount )/2 ) )
	{
		m_cbIsRequestOver = 0;
	...
	//所有玩家都处理了，关闭解散超时定时器
	if (nDealCancelCount == m_wChairCount)
	{
		KillGameTimer(IDI_TIME_CANCEL);
	}

	if(m_dwReplyCount < wOnlineCount && m_cbRequestReply[wChairID] == 2 ) return true;
	if(m_dwReplyCount < wOnlineCount && wAgreeCount <  ((m_wChairCount )/2  + 1 )&& m_wChairCount != 2)
	{
		return true;
	}

	if(wAgreeCount >= wOnlineCount || wAgreeCount >= ((m_wChairCount )/2  + 1 )&& m_wChairCount != 2) 
	{
		bCancel = true;
	}
