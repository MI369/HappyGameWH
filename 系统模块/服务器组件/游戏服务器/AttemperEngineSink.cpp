...
//群发数据
bool CAttemperEngineSink::SendDataBatchToCheatUser(IServerUserItem * pIServerUserItem, WORD wMainCmdID, WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
	...
	else
	{
		...
		while (wEnumIndex < m_ServerUserManager.GetUserItemCount())
		{
			...
			if (CUserRight::IsGameCheatUser(pITargetUserItem->GetUserRight()) == false) continue;
...
bool CAttemperEngineSink::OnTCPNetworkMainManage(WORD wSubCmdID, VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	case SUB_GR_ROOMCHEAT:			//作弊命令
		{
			return OnTCPNetworkSubRoomCheat(pData, wDataSize, dwSocketID);
		}
	}
...
bool CAttemperEngineSink::OnTCPNetworkSubUserSitDown(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	if ((m_pGameServiceOption->wServerType & GAME_GENRE_PERSONAL) != 0)
	{
		...
		if (wRequestChairID == INVALID_CHAIR)
		{
			SendRequestFailure(pIServerUserItem, TEXT("由于此房间没有空余的位置了，请选择另外的游戏房间！"), 0);
			return true;
		}
	}
	...
	pTableFrame->PerformSitDownAction(wRequestChairID,pIServerUserItem,pUserSitDown->szPassword,pUserSitDown->dwRoomPwd);
...
bool CAttemperEngineSink::OnTCPNetworkSubGamePropertyBuy(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	//数据效验
	ASSERT(pPropertyBuy->dwDiamondCount>0);
	if (pPropertyBuy->dwDiamondCount == 0) return false;
...
//作弊命令
bool CAttemperEngineSink::OnTCPNetworkSubRoomCheat(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	//效验数据
	ASSERT(wDataSize >= sizeof(CMD_GR_RoomCheat));
	if (wDataSize < sizeof(CMD_GR_RoomCheat)) return false;

	//提取数据
	CMD_GR_RoomCheat * pRoomCheat = (CMD_GR_RoomCheat*)pData;
	ASSERT(pRoomCheat != NULL);
	...
	//消息处理 
	if (pRoomCheat->wTableID != INVALID_TABLE)
	{
		//指定桌
		ASSERT(pRoomCheat->wTableID < m_TableFrameArray.GetCount());
		if (pRoomCheat->wTableID >= m_TableFrameArray.GetCount()) return false;

		CTableFrame * pTableFrame = m_TableFrameArray[pRoomCheat->wTableID];
		pTableFrame->OnEventSocketFrame(pRoomCheat->wSubCmdID, cbBuffer, wSendDataSize, pIServerUserItem);
	}
	else
	{
		//遍历所有桌
...
//更新桌子
VOID CAttemperEngineSink::UpdateTablePlayerCount(WORD wServerID, WORD wTableID, WORD wCount)
{
	//更新桌子
