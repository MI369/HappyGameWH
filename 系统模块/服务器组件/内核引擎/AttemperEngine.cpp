...
//网络前后台切换事件
bool CAttemperEngine::OnEventTCPNetworkForeground(DWORD dwSocketID, bool foreground)
{
	//缓冲锁定
	CWHDataLocker ThreadLock(m_CriticalLocker);
	...
	// 投递数据
	return m_AsynchronismEngine.PostAsynchronismData(EVENT_TCP_NETWORK_FOREGROUND, m_cbBuffer, sizeof(NTY_TCPNetworkForegroundEvent));
}
