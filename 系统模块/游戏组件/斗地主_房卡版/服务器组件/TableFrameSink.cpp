...
// 
	...
	for (WORD i=0;i<GAME_PLAYER;i++)
	{
		...
		//设置变量
		GameStart.lScore[i] = pHistoryScore->lCollectScore;
	}
	...
	case GER_NORMAL:		//常规结束
		{
			...
			for (WORD i=0;i<GAME_PLAYER;i++)
			{
				...
				//GameConclude.lScore[i] += GameConclude.lGameScore[i];

				GameConclude.lScore[i] = m_HistoryScore.GetHistoryScore(i)->lCollectScore;	
	...
	case GAME_SCENE_CALL:	//叫分状态
		{
			...
			for (WORD i=0;i<GAME_PLAYER;i++)
			{
				...
				/*TCHAR szMessage[128];
				_sntprintf(szMessage,CountArray(szMessage),TEXT("pStatusCall.lCollectScore = %ld"), StatusCall.lCollectScore[i]);
	...
	case 2000:
		{
			//////////////////////////////////////////////////////////////////////////
			//[波波]重置数据
			CTraceService::TraceString(TEXT("重置数据"), TraceLevel_Exception);
...
void CTableFrameSink::SetGameBaseScore(LONG lBaseScore)
{
	...
	//[波波]游戏局数置0
	CTraceService::TraceString(TEXT("游戏局数置0"), TraceLevel_Exception);
	m_cbGameIndex = 0;
}
