...
VOID CUserListControl::ConstructString(VOID * pItemData, WORD wColumnIndex, LPTSTR pszString, WORD wMaxCount)
{
	...
	case UD_GAME_ID:		//游戏标识
		{
			_sntprintf_s(pszString, wMaxCount, wMaxCount, TEXT("%ld"), pIClientUserItem->GetGameID());
			break;
		}
	case UD_USER_ID:		//用户标识
		{
			_sntprintf_s(pszString, wMaxCount, wMaxCount, TEXT("%ld"), pIClientUserItem->GetUserID());
			break;
		}
	case UD_NICKNAME:		//用户昵称
		{
			_sntprintf_s(pszString, wMaxCount, wMaxCount, TEXT("%s"), pIClientUserItem->GetNickName());
			break;
		}
	case UD_GENDER:			//用户性别
		{
	...
	case UD_BANKER:									//银行
		{
			_sntprintf_s(pszString, wMaxCount, wMaxCount, TEXT("%ld"), pIClientUserItem->GetUserInfo()->lInsure);
			break;
		}
	case UD_WIN_LOST:									//输赢情况
		{
			bool bAndroid = pIClientUserItem->IsAndroidUser();
			SCORE lScore = pIClientUserItem->GetUserScore() - pIClientUserItem->GetFirstScore();
			if (bAndroid) lScore = 0;

			_sntprintf_s(pszString, wMaxCount, wMaxCount, TEXT("%ld"), lScore);
			break;
		}
	case UD_IS_ANDROID:									//机器人？
		{
			bool bAndroid = pIClientUserItem->IsAndroidUser();
			lstrcpyn(pszString, (bAndroid == true) ? TEXT("机器人") : TEXT("玩家"), wMaxCount);
			break;
		}
	case UD_IS_CONTROL:									//是否控制
		{
			bool bAndroid = pIClientUserItem->IsAndroidUser();
			lstrcpyn(pszString, (bAndroid == true) ? TEXT("控制中") : TEXT("普通"), wMaxCount);
			break;
		}
	case UD_GROUP_NAME:		//社团名字
		{
			_sntprintf_s(pszString, wMaxCount, wMaxCount, TEXT("%s"), pIClientUserItem->GetGroupName());
			break;
		}
	case UD_UNDER_WRITE:	//个性签名
		{
			_sntprintf_s(pszString, wMaxCount, wMaxCount, TEXT("%s"), pIClientUserItem->GetUnderWrite());
			break;
		}
	case UD_TABLE:			//桌子号码
		{
			...
			if (wTableID != INVALID_TABLE) 
				_sntprintf_s(pszString, wMaxCount, wMaxCount, TEXT("%ld"), wTableID + 1);
			break;
		}
	case UD_CHAIR:			//椅子号码
		{
			WORD wChairID=pIClientUserItem->GetChairID();
			if (wChairID != INVALID_TABLE) _sntprintf_s(pszString, wMaxCount, wMaxCount, TEXT("%ld"), wChairID + 1);
			break;
		}
	case UD_SCORE:			//用户积分
		{
			_sntprintf_s(pszString, wMaxCount, wMaxCount, TEXT("%I64d"), pIClientUserItem->GetUserScore());
			break;
		}
	case UD_GRADE:			//用户成绩
		{
			_sntprintf_s(pszString, wMaxCount, wMaxCount, TEXT("%I64d"), pIClientUserItem->GetUserGrade());
			break;
		}
	case UD_USER_MEDAL:		//用户元宝
		{
			_sntprintf_s(pszString, wMaxCount, wMaxCount, TEXT("%ld"), pIClientUserItem->GetUserIngot());
			break;
		}
	case UD_EXPERIENCE:		//用户经验
		{
			_sntprintf_s(pszString, wMaxCount, wMaxCount, TEXT("%ld"), pIClientUserItem->GetUserExperience());
			break;
		}
	case UD_LOVELINESS:		//用户魅力
		{
			_sntprintf_s(pszString, wMaxCount, wMaxCount, TEXT("%ld"), pIClientUserItem->GetUserLoveLiness());
			break;
		}
	case UD_WIN_COUNT:		//胜局盘数
		{
			_sntprintf_s(pszString, wMaxCount, wMaxCount, TEXT("%ld"), pIClientUserItem->GetUserWinCount());
			break;
		}
	case UD_LOST_COUNT:		//输局盘数
		{
			_sntprintf_s(pszString, wMaxCount, wMaxCount, TEXT("%ld"), pIClientUserItem->GetUserLostCount());
			break;
		}
	case UD_DRAW_COUNT:		//和局盘数
		{
			_sntprintf_s(pszString, wMaxCount, wMaxCount, TEXT("%ld"), pIClientUserItem->GetUserDrawCount());
			break;
		}
	case UD_FLEE_COUNT:		//逃局盘数
		{
			_sntprintf_s(pszString, wMaxCount, wMaxCount, TEXT("%ld"), pIClientUserItem->GetUserFleeCount());
			break;
		}
	case UD_PLAY_COUNT:		//游戏局数
		{
			_sntprintf_s(pszString, wMaxCount, wMaxCount, TEXT("%ld"), pIClientUserItem->GetUserPlayCount());
			break;
		}
	case UD_WIN_RATE:		//用户胜率
		{
			FLOAT fWinRate=pIClientUserItem->GetUserWinRate();
			_sntprintf_s(pszString, wMaxCount, wMaxCount, TEXT("%4.2f%%"), fWinRate);
			break;
		}
	case UD_LOST_RATE:		//用户输率
		{
			FLOAT fLostRate=pIClientUserItem->GetUserLostRate();
			_sntprintf_s(pszString, wMaxCount, wMaxCount, TEXT("%4.2f%%"), fLostRate);
			break;
		}
	case UD_DRAW_RATE:		//用户和率
		{
			FLOAT fDrawRate=pIClientUserItem->GetUserDrawRate();
			_sntprintf_s(pszString, wMaxCount, wMaxCount, TEXT("%4.2f%%"), fDrawRate);
			break;
		}
	case UD_FLEE_RATE:		//用户逃率
		{
			FLOAT fFleeRate=pIClientUserItem->GetUserFleeRate();
			_sntprintf_s(pszString, wMaxCount, wMaxCount, TEXT("%4.2f%%"), fFleeRate);
			break;
		}
	case UD_GAME_LEVEL:		//游戏等级
		{
			...
			if (CServerRule::IsAllowAvertCheatMode(m_dwServerRule) && (pIClientUserItem->GetUserID()!=m_dwMySelfUserID))
				_sntprintf_s(pszString, wMaxCount, wMaxCount, TEXT("-"));
			else
				_sntprintf_s(pszString, wMaxCount, wMaxCount,TEXT("%s"), m_pIGameLevelParser->GetLevelDescribe(pIClientUserItem));
