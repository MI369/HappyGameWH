...
bool __cdecl CDataBaseSink::OnDataBaseEngineRequest(WORD wRequestID, DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	case DBR_GP_LOGON_VISITOR:			//游客登录	
		{
		return OnRequestLogon(wRequestID, dwContextID, pData, wDataSize);
		}
		...
		case DBR_GP_LOGON_VISITOR:			//游客登录	
			{
				...
				//执行查询
				DBR_GP_LogonVisitor * pLogonByVisitor = (DBR_GP_LogonVisitor *)pData;
				lReturnValue = SPLogonByVisitor(pLogonByVisitor->szPassWord, pLogonByVisitor->dwClientIP, pLogonByVisitor->szComputerID);
				pszPassword = pLogonByVisitor->szPassWord;
				break;
			}
		default :	//默认处理
			{
...
//游客存储过程
LONG CDataBaseSink::SPLogonByVisitor(LPCTSTR pszPassword, DWORD dwClientIP, LPCTSTR pszComputerID)
{
	//效验参数
	ASSERT(pszPassword != NULL);
