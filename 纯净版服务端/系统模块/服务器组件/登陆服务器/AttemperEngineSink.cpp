...
bool CAttemperEngineSink::OnSocketMainLogon(WORD wSubCmdID, VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	case SUB_GP_LOGON_USERID:		//ID 登录
		{
			return OnTCPNetworkSubLogonGameID(pData, wDataSize, dwSocketID);
		}
	case SUB_GP_REGISTER_ACCOUNTS:		//注册帐号
		{
			return OnTCPNetworkSubRegisterAccounts(pData, wDataSize, dwSocketID);
		}
	case SUB_GP_LOGON_VISITOR:      //游客登录
		{
		return OnTCPNetworkSubLogonVisitor(pData, wDataSize, dwSocketID);
		}
	}
	return false;
}
...
//I D 登录
bool CAttemperEngineSink::OnTCPNetworkSubLogonGameID(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	//效验参数
...
//帐号注册
bool CAttemperEngineSink::OnTCPNetworkSubRegisterAccounts(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	//效验参数
...
//游客登录
bool CAttemperEngineSink::OnTCPNetworkSubLogonVisitor(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	//效验参数
	...
	//处理消息
	CMD_GP_LogonVisitor * pLogonByVisitor = (CMD_GP_LogonVisitor *)pData;
	pLogonByVisitor->szPassWord[CountArray(pLogonByVisitor->szPassWord) - 1] = 0;
	...
	//效验版本
	if (pLogonByVisitor->dwPlazaVersion != VER_PLAZA_FRAME)
	{
		//获取版本
		WORD wLow = LOWORD(pLogonByVisitor->dwPlazaVersion);
		WORD wHigh = HIWORD(pLogonByVisitor->dwPlazaVersion);
	...
	//构造数据
	DBR_GP_LogonVisitor LogonByVisitor;
	memset(&LogonByVisitor, 0, sizeof(LogonByVisitor));
	LogonByVisitor.dwClientIP = dwClientAddr;
	lstrcpyn(LogonByVisitor.szPassWord, pLogonByVisitor->szPassWord, CountArray(LogonByVisitor.szPassWord));

	//变量定义
	VOID * pData = NULL;
	tagDataDescribe DataDescribe;
	CRecvPacketHelper RecvPacket(pLogonByVisitor + 1, wDataSize - sizeof(CMD_GP_LogonVisitor));
	...
	while (true)
	{
		...
		case DTP_COMPUTER_ID:		//机器标识
		{
			...
			if (DataDescribe.wDataSize == sizeof(tagClientSerial))
			{
				...
				for (INT i = 0; i < sizeof(tagClientSerial) / sizeof(BYTE); i++)
				{
					ASSERT(CountArray(LogonByVisitor.szComputerID) > ((i * 2) + 1));
					_stprintf(&LogonByVisitor.szComputerID[i * 2], TEXT("%02X"), *(pcbByte + i));
				}
			}
	...
	//投递数据库
	m_pIDataBaseEngine->PostDataBaseRequest(DBR_GP_LOGON_VISITOR, dwSocketID, &LogonByVisitor, sizeof(LogonByVisitor));
	return true;
}
