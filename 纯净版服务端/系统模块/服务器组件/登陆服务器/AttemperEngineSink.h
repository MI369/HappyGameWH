...
private:
	...
	//I D 登录
	bool OnTCPNetworkSubLogonGameID(VOID * pData, WORD wDataSize, DWORD dwSocketID);
	//帐号注册
	bool OnTCPNetworkSubRegisterAccounts(VOID * pData, WORD wDataSize, DWORD dwSocketID);
	//游客登录
	bool OnTCPNetworkSubLogonVisitor(VOID * pData, WORD wDataSize, DWORD dwSocketID);
	//用户信息处理
