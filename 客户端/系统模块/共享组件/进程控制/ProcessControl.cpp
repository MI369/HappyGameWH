...
//接口查询
VOID * __cdecl CProcessControl::QueryInterface(REFGUID Guid, DWORD dwQueryVer)
{
	...
	case IDT_ASYNCHRONISM:		//异步数据
		{
			...
			m_DataQueue.AddData(IDT_ASYNCHRONISM,&AsynchronismData,wHeadSize+AsynchronismData.wDataSize);
...
LRESULT	CProcessControl::OnMessageAsynchronismData(WPARAM wParam, LPARAM lParam)
{
	...
	tagAsynchronismData AsynchronismData;
	if (m_DataQueue.GetData(DataHead,&AsynchronismData,sizeof(AsynchronismData))==false)
	{
		ASSERT(FALSE);
		return 0;
	}
