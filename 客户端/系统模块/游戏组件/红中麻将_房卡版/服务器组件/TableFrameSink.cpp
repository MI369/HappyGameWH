...
CTableFrameSink::CTableFrameSink()
{
	...
	{
		m_lRoomCardInitialScore[i] = INVALID_LONGLONG;
	}
	...
	m_hVideoInst = NULL;
	m_pGameVideo = NULL;
	m_hVideoInst = LoadLibrary(TEXT("SparrowHZHGameVideo.dll"));
	if ( m_hVideoInst )
	{
		typedef void * (*CREATE)(); 
...
SCORE CTableFrameSink::QueryLessEnterScore(WORD wChairID, IServerUserItem * pIServerUserItem)
{
	...
	{		
		return 100 * m_pITableFrame->GetCellScore();
	}
	return 0;
}
...
bool CTableFrameSink::OnEventGameStart()
{
	...
	{
		if(!m_bPlayStatus[index])continue;

		BYTE cbCount = 0;
		CMD_S_Hu_Data HuData;
		ZeroMemory(&HuData,sizeof(HuData));

		m_GameLogic.AnalyseTingCard(m_cbCardIndex[index],0,0,HuData.cbOutCardCount,HuData.cbOutCardData,HuData.cbHuCardCount,HuData.cbHuCardData);

		if(index == m_wBankerUser)
		...
		for(int i=0;i<MAX_COUNT;i++)
		{
			if(HuData.cbHuCardCount[i]>0)
			{
				...
				{
					HuData.cbHuCardRemainingCount[i][j] = GetRemainingCount(index,HuData.cbHuCardData[i][j]);
				}
			}
			else
				break;
		...
		//特殊标记方便手机端处理
		if(index != m_wBankerUser && HuData.cbHuCardCount[0]>0)
			HuData.cbOutCardCount=1;

		m_pITableFrame->SendTableData(index,SUB_S_HU_CARD,&HuData,sizeof(HuData));
	...
	CopyMemory(GameStart.lRoomCardInitialScore, m_lRoomCardInitialScore, sizeof(m_lRoomCardInitialScore));
	...
	for (WORD i=0;i<m_cbPlayerCount;i++)
	{
		...
		if (m_pGameVideo)
		{
			...
			video.cbMagicIndex=m_cbMagicIndex;
			...
			CopyMemory(video.lRoomCardInitialScore, m_lRoomCardInitialScore, sizeof(m_lRoomCardInitialScore));
	...
	case GER_NORMAL:		//常规结束
		{
			...
			{
				GameConclude.cbMaData[i] = m_cbRepertoryCard[MAX_REPERTORY - m_cbMinusHeadCount-i-1];
			}

			GameConclude.cbLeftCardCount=m_cbLeftCardCount;

			for(int i=0;i<m_cbLeftCardCount;i++)
				GameConclude.cbLeftCardData[i]=m_cbRepertoryCard[MAX_REPERTORY-m_cbMinusHeadCount-i-1];
			...
			if( (m_pGameServiceOption->wServerType&GAME_GENRE_PERSONAL) !=0 )//房卡模式
			{
				...
				if (m_pGameVideo)
				{
					m_pGameVideo->AddVideoData(SUB_S_RECORD,&m_stRecord);
				}
			}
	...
	case GAME_SCENE_FREE:	//空闲状态
		{
			...
			//分配的初始积分都一样
			SCORE lRoomCardInitialScore[GAME_PLAYER];
			for (WORD i=0; i<GAME_PLAYER; i++)
			{
				lRoomCardInitialScore[i] = m_lRoomCardInitialScore[wChairID];
			}

			CopyMemory(StatusFree.lRoomCardInitialScore, lRoomCardInitialScore, sizeof(lRoomCardInitialScore));
	...
	case GAME_SCENE_PLAY:	//游戏状态
		{
			...
			CopyMemory(StatusPlay.lRoomCardInitialScore, m_lRoomCardInitialScore, sizeof(m_lRoomCardInitialScore));
...
bool CTableFrameSink::OnActionUserSitDown(WORD wChairID, IServerUserItem * pIServerUserItem, bool bLookonUser)
{
	...
	//房卡积分类型
	if (IsRoomCardScoreType() == true)
	{
		m_lRoomCardInitialScore[wChairID] = pIServerUserItem->GetUserScore();
	}
...
bool CTableFrameSink::OnUserOperateCard(WORD wChairID, BYTE cbOperateCode, BYTE cbOperateCard[3])
{
	...
	if ((wChairID != m_wCurrentUser) && (m_wCurrentUser != INVALID_CHAIR))  return true;
 	...
 	if (m_wCurrentUser == INVALID_CHAIR)
 	{
		...
		if(m_pGameVideo)
		{
			m_pGameVideo->AddVideoData(SUB_S_OPERATE_RESULT,&OperateResult);
		}
 		//设置用户
 		...
 		case WIK_CHI_HU:		//自摸
 			{
 				...
 				m_dwChiHuKind[wChairID] = m_GameLogic.AnalyseChiHuCard(m_cbCardIndex[wChairID], pWeaveItem, cbWeaveItemCount, m_cbSendCardData, m_ChiHuRight[wChairID],true);
...
bool CTableFrameSink::OnUserTrustee(WORD wChairID, bool bTrustee)
{
	...
	if(m_pGameVideo)
	{
		m_pGameVideo->AddVideoData(SUB_S_TRUSTEE,&Trustee);
	}
	if(bTrustee)
	{
		if(wChairID == m_wCurrentUser && m_bUserActionDone==false)
		{
			...
			if(cardindex==MAX_INDEX)//只剩红中了，自动胡
				return OnUserOperateCard(wChairID,WIK_CHI_HU,NULL);

			ASSERT(cardindex != MAX_INDEX);
			OnUserOutCard(wChairID,m_GameLogic.SwitchToCardData(cardindex),true);
		}
		else if(m_wCurrentUser == INVALID_CHAIR && m_bUserActionDone==false && m_cbUserAction[wChairID] != WIK_NULL)
		{
...
bool CTableFrameSink::SendOperateNotify()
{
	...
	for (WORD i=0;i<m_cbPlayerCount;i++)
	{
		if (m_cbUserAction[i]!=WIK_NULL)
		{
			...
			if(m_pGameVideo)
			{
				m_pGameVideo->AddVideoData(SUB_S_OPERATE_NOTIFY,&OperateNotify);
			}
		}
	}
...
BYTE CTableFrameSink::GetTimes(WORD wChairId)
{
	...
	{
		BYTE carddata = m_cbRepertoryCard[MAX_REPERTORY - m_cbMinusHeadCount-1];
	...
	else//2-6码
	{
		...
		{
			BYTE carddata = m_cbRepertoryCard[MAX_REPERTORY - m_cbMinusHeadCount-i-1];
...
bool CTableFrameSink::OnUserFlee(WORD wChairID)
{
	...
	if(m_pITableFrame->GetGameStatus() == GAME_SCENE_PLAY)
	{
		...
		ZeroMemory(&GameConclude, sizeof(GameConclude));
		GameConclude.cbSendCardData=m_cbSendCardData;
		GameConclude.wLeftUser = wChairID;
		...
		SCORE lLoseScore = (nCount-1)*m_pITableFrame->GetCellScore()*2*(nCount-1);
		...
		for(WORD i=0;i<m_cbPlayerCount;i++)
		{
			...
			if(i != wChairID)
			{
				GameConclude.lGameScore[i] += ( lLoseScore/(nCount-1) );
