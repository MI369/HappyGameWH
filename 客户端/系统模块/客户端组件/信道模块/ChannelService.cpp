...
LRESULT CChannelService::OnIPCMessage(WPARAM wParam, LPARAM lParam)
{
	...
	ASSERT(DataHead.wDataSize==IPCPacket.Head.wPacketSize +IPCPacketSize(IPCPacket));
	if (DataHead.wDataSize<IPCPacketSize(IPCPacket)) return 0;
	if (DataHead.wDataSize!=IPCPacket.Head.wPacketSize +IPCPacketSize(IPCPacket)) return 0;
	...
	try
	{
		...
		else	//外围消息
		{
			...
			{
				WORD wDataSize=IPCPacket.Head.wPacketSize;
